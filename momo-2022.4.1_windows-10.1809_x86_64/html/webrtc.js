...
//       let needData = gamepad.axes.slice(0,7); 
//       //console.log("Button Values:", gamepad.buttons.map(button => button.value));
      

//       let x=ValueAdjust(needData[0]);
//       let y=ValueAdjust(needData[1]);
//       let SpeedLimit=ControlSpeed(needData[6]);
...
function sendGamepadData() {
  ...
  for (let index in controllers) {
      let gamepad = controllers[index];
      let needData = gamepad.axes.slice(0, 7);
      let x = ValueAdjust(needData[0]);
      let y = ValueAdjust(needData[1]);
      let SpeedLimit=ControlSpeed(needData[6]);
      allGamepadData.push({ x: x, y: y ,limit:SpeedLimit});
  }
  if (dataChannel && dataChannel.readyState === "open") {
      dataChannel.send(JSON.stringify(allGamepadData));
  }
}


function ValueAdjust(speed){
  const MAX=-1;
  const DeadZone=100;
  speed*=MAX;
  speed*=1000;
  ...
  //speed/=1000;
  if(speed > -1*DeadZone && speed < DeadZone){
    return 0;
  }else{
    return speed;
  }
